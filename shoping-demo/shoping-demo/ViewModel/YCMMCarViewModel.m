...
@implementation YCMMCarViewModel
...
- (void)getData{
    ...
    for (int i = 0; i<allCount; i++) {
        ...
        for (int x = 0; x<goodsCount; x++) {
            YCMMCarModel *cartModel = [[YCMMCarModel alloc] init];
...
- (float)getAllPrices{
    ...
    NSArray *pricesArray = [[[self.cartData rac_sequence] map:^id(NSMutableArray *value) {
        return [[[value rac_sequence] filter:^BOOL(YCMMCarModel *model) {
            if (!model.isSelect) {
                self.isSelectAll = NO;
            }
            return model.isSelect;
        }] map:^id(YCMMCarModel *model) {
...
- (void)selectAll:(BOOL)isSelect{
    ...
    self.cartData = [[[[self.cartData rac_sequence] map:^id(NSMutableArray *value) {
        return  [[[[value rac_sequence] map:^id(YCMMCarModel *model) {
...
- (void)rowSelect:(BOOL)isSelect IndexPath:(NSIndexPath *)indexPath{
    ...
    YCMMCarModel *model         = goodsArray[row];
    [model setValue:@(isSelect) forKey:@"isSelect"];
    //判断是都到达足够数量
    NSInteger isSelectShopCount = 0;
    for (YCMMCarModel *model in goodsArray) {
        if (model.isSelect) {
            isSelectShopCount++;
        }
    }
...
- (void)rowChangeQuantity:(NSInteger)quantity indexPath:(NSIndexPath *)indexPath{
    ...
    YCMMCarModel *model = self.cartData[section][row];
...
- (void)deleteGoodsBySingleSlide:(NSIndexPath *)path{
    ...
    if (shopArray.count == 0) {
        ...
        for (YCMMCarModel *model in shopArray) {
            if (model.isSelect) {
                isSelectShopCount++;
            }
        }
        [self.shopSelectArray replaceObjectAtIndex:section withObject:@(isSelectShopCount==shopCount?YES:NO)];
        [self.cartTableView reloadSections:[NSIndexSet indexSetWithIndex:section] withRowAnimation:UITableViewRowAnimationFade];
    }
